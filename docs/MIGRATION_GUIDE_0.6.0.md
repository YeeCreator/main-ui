...
**主包版本对齐**：发布定稿时 `main-ui` 主包从 `0.4.0` 升至 `0.6.0`（与模板库世代号对齐），全部 `@main-ui/*` 子包的 `main-ui` peer 同步为 `^0.6.0`。仅版本号与 peer 范围变化，导出面与行为无任何差异。

## 包版本矩阵

| 包 | 0.5.0 | 0.6.0 | 说明 |
|---|---|---|---|
| `main-ui` | 0.4.0 | **0.6.0** | 主包版本与世代对齐（导出面不变） |
